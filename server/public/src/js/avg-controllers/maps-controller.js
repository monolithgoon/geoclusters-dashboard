...
const LeafletMapsSetup = ((baseMap, featDetailMap) => {
	const { googleStreets, googleHybrid, osmStd, osmBW, mapboxOutdoors, bingMapsArial } =
		LEAFLET_TILE_LAYERS;
 ...
 */
	...
	(function addCustomControls(map) {
		...
		L.control
			.betterscale({ position: "basemap-controls-placeholder", metric: true, imperial: false })
			.addTo(map);
	})(baseMap);
})(AVG_BASE_MAP, FEAT_DETAIL_MAP);
	...
	return {
		...
		getFeatPropsMarkup: (featProps) => {
            ...
            <div class="metadata-label--owner-info__avg"> 
               ...
               <span> ${_.startCase(
						_joinWordsArray(Object.values(featProps.featureAdmin.admin1.names))
					)} </span>
            </div>
            <div class="metadata-label--turn-by-turn" id="metadata_label_turn_by_turn">
               <a href="#" role="button" title="Plot boundary turn-by-turn directions" aria-label="Plot boundary turn-by-turn directions"></a>
			...
			try {
				switch (_TurfHelpers.getType(gjFeature)) {
					...
					case "GeometryCollection":
						throw new Error(
							`Cannot create marker for this feature: ${gjFeature} of type ${_TurfHelpers.getType(
								gjFeature
							)}`
						);
					...
					default:
						...
						// RENDER OUTLINE
						if (featGeometry) {
							LeafletMaps.getFeatPolyOutline(featGeometry, {
								lineColor,
								lineWeight,
								lineOpacity,
								lineDashArray,
		...
		drawPolySurveyDetails: (
			gjPolygon,
			polyLayerGroup,
			{ distThresh = 10, bearingThresh = 45 }
		) => {
			...
			try {
				...
				(function renderBoundsDetails() {
					...
					for (let idx = 0; idx < polyCoords.length; idx++) {
						...
						const toPlotCorner =
							polyCoords[idx + 1] === undefined ? polyCoords[0] : polyCoords[idx + 1]; // RETURN BACK TO STARTING CORNER
						...
						if (turfBearing !== 0 && distance > distThresh) {
							...
							L.marker([plotCorner[1], plotCorner[0]], {
								icon: L.divIcon({
									...
									html: `<span>${idx}</span>${plotCorner[0].toFixed(
										6
									)}°N, ${plotCorner[1].toFixed(6)}°E`,
									iconSize: [100, 30],
								}),
								zIndexOffset: 98,
...
const MapboxMaps = ((subplotsMap) => {
	return {
		...
		panToCoords: (
			...
			centerCoords,
			bounds,
			{ zoom = 16, pitch = 0, bearing = 0, boundsPadding = 0 }
		) => {
			// PAN TO LOCATION
			map.flyTo({
				center: centerCoords,
		...
		const layerClick = (map, fillLayer) => {
			map.on(`click`, `${fillLayer.id}`, (e) => {
				...
				(function renderBoundsDetails(featureData, leafletMap, leafletLayerGroup) {
					...
					LeafletMaps.getFeatPolyOutline(featGeometry, { lineWeight: 4 }).addTo(
						leafletLayerGroup
					);
					...
					LeafletMaps.getClusterPlotHTMLMarker(featProps, featCenter, "plot-metadata-label", {
						draggable: true,
					...
					for (let idx = 0; idx < featCoords.length; idx++) {
						...
						const toPlotCorner =
							featCoords[idx + 1] === undefined ? featCoords[0] : featCoords[idx + 1]; // RETURN BACK TO STARTING CORNER
...
export const _RenderEngine = (function (avgBaseMap, clusterFeatsMap) {
	try {
		...
		const sidemapPanToCluster = async (featColl) => {
			...
			MapboxMaps.panToCoords(clusterFeatsMap, gjCenterCoords, gjBounds, {
				zoom: 16,
				pitch: 0,
				bearing: 0,
				boundsPadding: 0,
		...
		return {
			...
			renderFeatColl: (
				...
				for (let idx = 0; idx < gjFeatColl.features.length; idx++) {
					...
					LeafletMaps.drawFeature(gjFeature, {
						useBuffer,
						bufferAmt,
						bufferUnits,
						lineColor,
			...
			renderClusterPoly: (featCollPoly, { bufferUnits } = {}) => {
				try {
					...
					if (featCollPoly && _TurfHelpers.getType(featCollPoly) === "Polygon") {
						...
						const clusterMetaLabelsLG = LLayerGroupController.initLayerGroup(
							"cluster-metadata-labels",
							{ visibilityRank: 2 }
						);

						// DISPLAY CLUSTER DETAILS IN HTML MARKER
						LeafletMaps.getClusterMetadataMarker(
							polyProps,
							polyCenter,
							"plot-metadata-label",
							{ draggable: true }
						...
						const clusterPolyOutlineLG = LLayerGroupController.initLayerGroup(
							polyProps.clusterID,
							{ visibilityRank: 5 }
						);

						// RENDER CLUSTER SURVEY DETAILS
						LeafletMaps.drawPolySurveyDetails(featCollPoly, clusterPolyOutlineLG, {
							bearingThresh: 45,
							distThresh: 30,
						});
						...
						if (bufferedBboxPoly)
							finalBboxPoly = _ProcessGeoJSON.getBboxPoly(bufferedBboxPoly);

						// RENDER CLUSTER BBOX SURVEY DETAILS
						if (finalBboxPoly) {
							// INIT. LAYER GROUP FOR BBOX
							const bboxLayerGroup = LLayerGroupController.initLayerGroup(
								polyProps.clusterID,
								{ visibilityRank: 4 }
							);
							...
							LeafletMaps.drawFeature(finalBboxPoly, {
								featLayerGroup: bboxLayerGroup,
								lineColor: "cyan",
								lineWeight: 0.55,
							});

							LeafletMaps.drawPolySurveyDetails(finalBboxPoly, bboxLayerGroup, {
								bearingThresh: 0,
								distThresh: 5,
							});
						}
			...
			populateClustersOnBasemap: async (
				featureCollections,
				{ useBuffer, bufferAmt, bufferUnits } = {}
			) => {
				const zoomLevel = LeafletMaps.getMapZoom();
				...
				if (featureCollections.length > 0) {
					for (let idx = 0; idx < featureCollections.length; idx++) {
						...
						if (featColl.features.length > 0) {
							...
							if (clusterPolygon)
								_RenderEngine.renderClusterPoly(clusterPolygon, { bufferUnits });
							...
							for (let idy = 0; idy < featColl.features.length; idy++) {
								...
								const featMarker = LeafletMaps.initMarker(feature, {
									useBuffer,
									bufferAmt,
									bufferUnits,
								});
			...
			renderSidemapClusterPlotsLabels: async (
				...
				if (featColl.features) {
					for (let idx = 0; idx < featColl.features.length; idx++) {
						...
						await MapboxMaps.drawGJPolygonLabel(clusterFeatsMap, clusterPlotFeat, idx, {
							areaUnits,
						});
					}
				} else {
...
const AffectLeafletMarker = (() => {
	...
	var marker = new L.Marker([36.8370066107919, 10.059871561852127])
		.bindPopup("Popup")
		.addTo(AVG_BASE_MAP);
