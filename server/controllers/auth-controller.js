...
exports.logout = catchAsync(async (req, res, next) => {
	...
	// Obtain the current user from the req. object appended by the protectRoute fn.
	const currentUser = req.user;

   /**
      In order to logout the user, the The original cookie "jwtcookie needs to be deleted from the browser"
      However, because the 'httpOnly' option = 'true', the cookie cannot be modified or deleted directly in the browser code
      This response overrides the original cookie by sending out a dummy token with the same name
      This action logs out the user 
    */
   res.cookie(`jwtcookie`, `dummy_logged_out_token`, {
      expires: new Date(Date.now() + (10 * 1000)),
      httpOnly: true,
   })

	// Send a success message to indicate that the user has logged out
	res.status(200).json({
		status: `success`,
...
exports.protectRoute = catchAsync(async (req, res, next) => {
	...
	if (!currentUser) {
		return next(
			new ServerError(
				`The user that owns those login credentials no longer exists.`,
				401,
				`protectRouteFn.`
			)
		);
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
	...
	if (!dbUser) {
		return next(
			new ServerError(
				`There is no user with that email address [ ${req.body.user_email} ].`,
				404,
				`forgotPasswordFn`
			)
		);
	...
	try {
		...
		return next(
			new ServerError(
				`There was an error sendng the password reset email. Check your internet connection. Try again later. ${sendEmailErr.message}`,
				500,
				`forgotPasswordTryCatch`
			)
		);
	}
}, `forgotPasswordFn`);
...
exports.resetPassword = catchAsync(async (req, res, next) => {
	...
	if (!dbUser) {
		return next(new ServerError(`The token is invalid or has expired`, 400, `resetPasswordFn`));
	}
